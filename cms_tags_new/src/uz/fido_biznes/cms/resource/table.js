...
, _tblLang = ["нет данных"], _oc_, _lastTime = 0, _chMenu=[{label:"Выделить все",action:function(){_cA_(1)}},{label:"Отменить выделения",action:function(){_cA_(2)}},{label:"Инвертировать выделения",action:function(){_cA_(3)}}],_chc_;
function _fc(h, r)
{
   function dc(h)
   {
      ...
      {
         var _sn="",_s = "<input name=f" + h.i + " id=f" + h.i + (r == 2 ? " onkeypress='if(event.keyCode==13){go({param:{f"+h.i+":this.getValue()}})}'" : "") + (is.def(h.f.c) ? " size=" + h.f.c : "") + (is.def(h.f.m) ? ' mask="' + h.f.m + '"' : "");
        if (r!=2) {
          if (h.f.rf) {
   ...
   if (h.f)
   {
      ...
      switch (h.f.t)
      {
         ...
         case 2 :
            ...
            break;
         case 3 :
            s += "<select name=fo" + h.i + '><option value="=">равно<option value=">">больше<option value="<">меньше<option value="_like_" selected>содержит<option value="like_">начиная с<option value="_like">с конца</select>';
            s += dc(h);
      }
...
function showFilter()
{
   ...
   s += '<input type=button value="Применить" onclick="applyFilter()"><input type=reset value="Очистить"></table>';
   s += '<table><tr style="font:bold"><td colspan=2 align=center>Поля, подлежащие фильтрации<td align=center>Сортировка';
   ...
   var r=go({target:"modalE",action:function(w){
    w._.body.innerHTML=s;
    w._.body.style.background = w._.body.currentStyle["modal-background"];
    w.data = {fm:nvl(tdd.w,{})};
    w.tdf = tdf;
    w.sort = nvl(tdd.s,[]);
    w.evalSize(w.base.clientWidth, w.base.clientHeight);
    w.findIt=function(v)
    {
      for(var i=0;i<w.sort.length;i++)if(w.sort[i][0]==v)return i;
      return - 1
    };
   ...
   },lock:false,dialogWidth:1100,dialogScroll:"yes"});
   if (r!=null) go({param:r});
}
...
(function()
{
   ...
   if (tdd.p || ! is.hasFlag(tdf.f, 16))
   {
      ...
      if (tdd.p)
      {
         ...
         st += '<button class=navbut onclick="goPage(\'first\')"' + (tdd.p.PN <= 1 ? 'disabled' : '') + '>9</button><button class=navbut onclick="goPage(\'prev\')"' + (tdd.p.PN <= 1 ? 'disabled' : '') + ">7</button>";
         ...
         st += '<button class=navbut onclick="goPage(\'succ\')"' + (tdd.p.PN >= tdd.p.MP ? 'disabled' : '') + '>8</button><button class=navbut onclick="goPage(\'last\')"' + (tdd.p.PN >= tdd.p.MP ? "disabled" : "") + ">:</button>";
      }
      if ( ! is.hasFlag(tdf.f, 16))
      st += '<button class=navbut onclick="go({})">q</button>';
      if ( is.hasFlag(tdf.f, 32))
      st += '<button class=navbut onclick="go({param:{_recreateValue:1}})">x</button>';
      if ( ! is.hasFlag(tdf.f, 64) && is.hasFlag(tdd.f, 1))
      {
         st += '<button onclick="showFilter()"' + (is.hasFlag(tdd.f, 2) ? " class=withFilter" : "") + ">Фильтр</button>";
      }
   }
   if (st != "")
   {
      try
      {
         getDOM("tableControls").innerHTML = st + getDOM("tableControls").innerHTML;
      }
      catch(ex)
      {
   ...
   st += '<div class=panel id=basepanel style="height:300px"><form name=tblForm nocycle=1 method=post><table id=tbl cellspacing=0 cellpadding=5 cl=' + k
   + "><thead><tr>";
      ...
      if(tdf.c[i].t != 4)
      {
         ...
         if(tdf.c[i].t != 2) {
           if(dataExist() && ! is.hasFlag(tdf.f, 8))
           {
              ...
              st += '<th onclick="go({param:{s'+tdf.c[i].i+":[1,'"+(dr1==6?"desc":"")+'\']}})">';
           }
           else
           {
              st += "<th>";
           ...
           st +='><button onclick="_chc_='+i+';drawMenu(_chMenu,event.clientX, event.clientY)" style="width:12px;height:12px">&nbsp;</button>';
         }
   ...
   if (tdd.d.length == 0)
   {
      ...
      st += "<tr class=cellEven><td colspan=" + k + " align=center>" + _tblLang[0];
   }
   st += "</table></form></div>";
   if (tdf.t)
   {
      st += '<fieldset style="clear:left"><form name=footForm><table width=100%>';
      for (var i = 0; i < tdf.t.length; i ++ )
      {
         st += "<tr>";
         for (var j = 0; j < tdf.t[i].length; j ++ )
         {
            st += '<td style="width:1px"';
...
function moveCell(cell)
{
   if (dataExist())
   {
      ...
      var row = _oc_.parentElement, nc = _oc_.firstChild, row2 = cell.parentElement,oc;
      if (nc.stopMoving && event.type != "click")
      return;
      _oc_.className = "";
      ...
      _oc_ = cell;
      nc = cell.firstChild;
      if(nc.tagName == "INPUT" && nc.type == "text")
      nc.focus();
      else
      cell.focus();
      ...
      if(tdf.t)
      {
         for (var i = 0; i < tdf.t.length; i ++ )
         for (var j = 0; j < tdf.t[i].length; j ++ )
         _.footForm["a" + tdf.t[i][j].i].value = getData(tdf.t[i][j].i)
      }
      ...
      */
      ...
      if((row != row2 && dataExist() || event.type == "load") && typeof(onRowChange) == "function")
      {
         _lastTime = (new Date()).getTime();
         setTimeout("if((new Date()).getTime()-_lastTime >= 300)onRowChange()", 300);
      }
...
window.funcLoad.push(function()
{
   tbl.onclick = tbl.ondblclick = function()
   {
      var o = event.srcElement, nc;
   ...
   tbl.onkeydown = function()
   {
      ...
      function w(d, e)
      {
         var r = _oc_.parentNode.sectionRowIndex, c = _oc_.cellIndex,
         rl = tbl.tBodies[0].rows.length, cl = tbl.rows[0].cells.length, s = 0;
         function rd(d)
         {
            r += d;
            s ++ ;
   ...
   var kw = [], sum = 0, w2;
   for (var i = tbl.cl; i < tbl.tHead.rows[0].cells.length;
   i ++ )
   {
