 ...
 * _.write("Hello World!");
 */
var _ = document,_locklayer;
function IEVersion(){
try{return Number((/(msie) ([\w.]+)/.exec(navigator.userAgent.toLowerCase())||[])[2]||"0")}catch(e){return 6}
}
...
function go(D)
{
  ...
  */
  if (/modal/.test(D.target))
  {
    ...
    var fh = _.createElement("<input type=hidden>"), h, pr;
    if(is.undef(D.form))
    {
...
function formatNumber(v, sc)
{
  ...
  v = v.replace(/[,.]/, "@").replace(/[,.]/g, "").replace(/@/, ".").replace(/[^0-9.]/g, "").replace(/^[0]+/, "");
  if (v == "")v = "0";
  if (v.charAt(0) == ".")v = "0" + v;
  if(sc)
  {
    ...
    if (sc != - 1)
    {
      ...
      else for(var i = v[1].length; i < sc; i ++ ) v[1] = v[1] + "0";
    }
    else
    {
      var i = v[1].length - 1;
...
function callRequest(t)
{
  ...
  var lastError = nvl(t.error, 0), el;
  t.error = 0;
  if(t.fill)t.fill();
...
(function()
{
  ...
  */
  ...
  function maskkeydown()
  {
    ...
    if(t.cm.type == "mask")
    {
      var key = event.keyCode, selStart = t.getStart(), selEnd = t.getEnd();
      t.setValue(t.value);
  ...
  function maskkeypress()
  {
    ...
    if(t.cm.type == "mask")
    {
      ...
      var newChar = String.fromCharCode(event.keyCode), pos = t.getStart();
      ...
      typeof(t.chars[pos]) != "string" && ! (newChar.match(t.chars[pos]) ? true : ((newChar = newChar.toUpperCase()).match(t.chars[pos])) ? true : ((newChar = newChar.toLowerCase()).match(t.chars[pos])) ? true : false) )
      {
        t.setValue(t.value);
        t.setCaret(pos)
      }
  ...
  */
  window.initElement = function(t)
  {
    ...
    if (t.enable)
    {
      var sOR, sAND, m, n, vED, enb = t.enable, se, si = 0;
      se = eval(t.form.name + "." + t.name);
      if(se != t)
      {
        while(se[si] != t) si ++ ;
      ...
      {
        var t = this, s = false, sOR, sAND, el, si = 0;
    ...
    else if((t.tagName == "INPUT" && (t.type == "submit" || t.type == "button" || t.type == "reset")) || t.tagName == "BUTTON")
    {
      ...
      t.onmouseout = function(){this.style.background=this.currentStyle["button-background"]};
      t.onmouseover = function(){this.style.background=this.currentStyle["button-background-hover"]};
      if (t.tagName == "BUTTON")
      {
        t.setValue = function(v)
        {
    ...
    else if (t.tagName == "FORM")
    {
      ...
      {
        var e = event.srcElement, k = event.keyCode;
        if(k != 13 ||
        e.type == "submit" ||
        e.type == "button" ||
        e.type == "reset" ||
      ...
      t.check = function()
      {
        var k = true, F, t = this, el = t.elements, e;
  ...
  window.initDOMShowHide = function (t)
  {
    if(t.showhide)
    {
      var sOR, sAND, m, n, vSH, shd = t.showhide, se, w;
      sOR = shd.split("||");
      t.SH = [];
  ...
  if (/[?&]modal/.test(_.URL)){
    top._t = function (){return window.dialogArguments.opener.top._t()};
    _.write("<title>");
    var m = top._t().modalTitle;
...
function evalDivSize(d, s)
{
  ...
  var r = t.getClientRects()(0), clHeight = _.body.clientHeight - _.body.scrollHeight + r.bottom - r.top - 5;
    ...
    else
    {
      ...
      for (var elv in val)
      {
        ...
        if (is.def(v[0]))
        {
          ...
          if (c < 0)
          {
            alert("fillForm: Error in form values"); throw "err";
          }
          if (e.mbid)
          {
            b = getDOM(e.mbid);
        ...
        }catch(e){alert("Error:"+elv);throw e}
      }
    }
  }
  enableElements();
...
function drawTab(tab,asTab)
{
  ...
  function showMenu(t)
  {
    ...
    for(var v in tab){
      if(ti.tag.items){_.body.focus();break}
    }
    if (t.tag.items)
    {
      drawMenu( t.tag.items
              , t.offsetParent.offsetLeft + t.offsetLeft
              , t.offsetParent.offsetTop + t.offsetTop + t.offsetHeight + 3
              , t);
    }
  }
...
function printPreview(url){
  go({url: url, target:'new', lock:false, arg:"left=0, top=0, width="+(screen.availWidth-10)+", height="+(screen.availHeight-70)+", scrollbars=1,directories=0,location=0,menubar=1,resizable=1,status=1,titlebar=0,toolbar=0"});
}
